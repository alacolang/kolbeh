...
export const userStatsPerCategory = (users: User[]): number[] => {
    return users.map((user) => happinessTrainingData.categories.map((category) => {
        return category.exercises.every((exercise) =>
...
export const triedThisWeek = (users: User[]): number => {
    return users.filter((user) => (differenceInDays(new Date(), user.modifiedAt) <= 7)).length;
};

export const didExerciseThisManyTimes = (users: User[]) => (howManyTimes: number): number => {
    return users.map((user) => happinessTrainingData.categories.map((category) => {
    ...
    }, 0)).reduce((acc, userTotalNumberExercisesDone) => {
...
export const howManyUsersDidExerciseLastWeek = (users: User[]): number => {
    return users.map((user) => happinessTrainingData.categories.map((category) => {
        return category.exercises.filter((exercise): number => {
            ...
            const didExerciseLastWeek = ((differenceInDays(new Date(), exerciseDoneTime) > 7) &&
             (differenceInDays(new Date(), exerciseDoneTime) <= 14)) ? 1 : 0;
            return didExerciseLastWeek;
        }).length
    }).reduce((acc, TimesDidExerciseLastWeek): number => {
        return acc + TimesDidExerciseLastWeek;
    }, 0)).reduce((acc, TotalTimesUserDidExerciseLastWeek) => {
...
statsRouter.get("/", (req, res) => {
    findAll().then((users) => {
        res.json({
            ...
            howManyExcercisesUsersDidLastWeek: howManyExcercisesUsersDidLastWeek(users),
        });
    });
    // res.json({ status: "ok" });
})
